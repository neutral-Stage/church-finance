...
export function ChurchProvider({ children }: ChurchProviderProps) {
  ...
  const [selectedChurch, setSelectedChurchState] =
    useState<ChurchWithRole | null>(null);
      ...
      if (typeof window !== "undefined") {
        ...
        if (stored) {
          try {
            ...
            if (DEV_MODE)
              console.log(
                "[ChurchContext] Loaded church from localStorage:",
                parsedChurch.id,
              );
          } catch (error) {
            console.error(
              "[ChurchContext] Error parsing stored church:",
              error,
            );
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (
              data.church &&
              (!storedChurch || data.church.id !== storedChurch.id)
            ) {
              const justSynced =
                sessionStorage.getItem("churchSyncCompleted") === "true";

              if (justSynced && storedChurch) {
                if (DEV_MODE)
                  console.log(
                    "[ChurchContext] Server differs but client just synced. Trusting client:",
                    storedChurch.id,
                  );
                ...
                if (DEV_MODE)
                  console.log(
                    "[ChurchContext] Server has different church, syncing to client:",
                    data.church.id,
                  );
              ...
              if (!hasSynced) {
                ...
                if (syncResponse.ok) {
                  ...
                  console.log(
                    "[ChurchContext] Refreshing page to refresh server components",
                  );
                  router.refresh();
                } else {
                  console.error(
                    "[ChurchContext] Failed to sync cookie to server:",
                    syncResponse.status,
                  );
                }
              } else {
                console.log(
                  "[ChurchContext] Already attempted sync this session, skipping to prevent loop",
                );
              }
            } else if (
              data.church &&
              storedChurch &&
              data.church.id === storedChurch.id
            ) {
              // Server and client are already in sync
              if (DEV_MODE)
                console.log(
                  "[ChurchContext] Server and client already in sync:",
                  storedChurch.id,
                );
              sessionStorage.setItem("churchSyncCompleted", "true");
            } else if (!data.church && !storedChurch) {
              ...
              if (DEV_MODE)
                console.log(
                  "[ChurchContext] No church selected on server or client",
                );
            }
          } else {
            console.error(
              "[ChurchContext] Failed to fetch server church selection:",
              response.status,
            );
          }
        } catch (error) {
  ...
  const fetchChurches = useCallback(async () => {
    try {
      ...
      const response = await churchApi.get("/api/churches", {
        skipChurchValidation: true,
      });
  ...
  const setSelectedChurch = useCallback(
    async (church: ChurchWithRole | null, skipReload = false) => {
      ...
      if (DEV_MODE)
        console.log(
          "[ChurchContext] Changing church from",
          currentChurchId,
          "to",
          newChurchId,
        );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (!skipReload && typeof window !== "undefined") {
            // Use router.refresh() instead of full page reload for smoother UX
            if (DEV_MODE)
              console.log(
                "[ChurchContext] Refreshing page with new church context",
              );
            router.refresh();
          } else {
            if (DEV_MODE)
              console.log(
                "[ChurchContext] Cookie synced, skipping reload (auto-select)",
              );
            setIsUpdating(false);
          }
        }
  ...
  useEffect(() => {
    const syncSelectedChurch = async () => {
      ...
      if (availableChurches.length > 0 && selectedChurch) {
        const churchStillExists = availableChurches.find(
          (c: ChurchWithRole) => c.id === selectedChurch.id,
        );
        if (!churchStillExists) {
          if (DEV_MODE)
            console.log(
              "[ChurchContext] Selected church no longer exists, auto-selecting first church",
            );
        ...
        if (DEV_MODE)
          console.log(
            "[ChurchContext] No church selected, auto-selecting first church",
          );
