...
// Reads cookies via createServerClient on each request — must be dynamic.
export const dynamic = 'force-dynamic';

// Lazy-init the admin client so a missing SUPABASE_SECRET_KEY doesn't kill
// cold starts or trip build-time evaluation.
let _adminSupabase: ReturnType<typeof createAdminClient> | null = null;
function getAdminSupabase() {
  if (!_adminSupabase) _adminSupabase = createAdminClient();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: attachmentResult, error: attachmentError } = await (getAdminSupabase()
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data: attachmentResult, error: updateError } = await (getAdminSupabase()
      .from('document_attachments') as any)
      .update(updateData)
      .eq('id', id)
      .select()
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { data: existingAttachment, error: fetchError } = await (getAdminSupabase()
      .from('document_attachments') as any)
      .select('*')
      .eq('id', id)
      .single();
