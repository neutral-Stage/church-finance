...
export async function POST(request: NextRequest) {
  try {
    ...
    const { church_id: bodyChurchId } = body
    ...
    const billData = {
      ...
      notes: notes || null,
      frequency,
      ...(bodyChurchId ? { church_id: bodyChurchId } : {}),
    };
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Defense-in-depth: scope update to the church the bill belongs to
    const billChurchId = (currentBill as any).church_id
    if (billChurchId) updateData.church_id = billChurchId
...
export async function DELETE(request: Request) {
  try {
    ...
    // Verify the bill belongs to a church the caller can access
    const { data: billCheck } = await safeSelect(supabase, 'bills', { filter: { column: 'id', value: id } })
