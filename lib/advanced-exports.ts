...
export class AdvancedPDFExporter {
  ...
  generateNarrativeReport(narrative: {
    ...
    this.doc.setFontSize(10)
    this.doc.setTextColor(156, 39, 176)
    this.doc.text('AI-Generated Narrative Report', this.doc.internal.pageSize.width / 2, 55, { align: 'center' })
    this.doc.setTextColor(0, 0, 0)

    let currentY = 70

    // Add each narrative section
    const sections = [
      { title: 'Executive Summary', content: narrative.executiveSummary, color: [33, 150, 243] },
      { title: 'Income Analysis', content: narrative.incomeAnalysis, color: [76, 175, 80] },
      { title: 'Expense Analysis', content: narrative.expenseAnalysis, color: [244, 67, 54] },
      { title: 'Fund Performance', content: narrative.fundPerformance, color: [156, 39, 176] },
      { title: 'Trends & Insights', content: narrative.trendsAndInsights, color: [255, 152, 0] },
      { title: 'Recommendations', content: narrative.recommendations, color: [0, 188, 212] }
    ]
    ...
    sections.forEach((section, index) => {
      ...
      this.doc.setFillColor(section.color[0], section.color[1], section.color[2])
      this.doc.rect(this.margin, currentY - 5, this.doc.internal.pageSize.width - (this.margin * 2), 10, 'F')

      this.doc.setFontSize(12)
      this.doc.setFont('helvetica', 'bold')
      this.doc.setTextColor(255, 255, 255)
      this.doc.text(section.title, this.margin + 3, currentY + 2)
      this.doc.setTextColor(0, 0, 0)
      ...
      const splitText = this.doc.splitTextToSize(section.content, maxWidth)

      // Check if content will overflow page
      const textHeight = splitText.length * 5
      if (currentY + textHeight > this.pageHeight - 30) {
        // Split content across pages if needed
        let remainingText = [...splitText]
        while (remainingText.length > 0) {
          const availableLines = Math.floor((this.pageHeight - 30 - currentY) / 5)
          const pageText = remainingText.slice(0, availableLines)
          remainingText = remainingText.slice(availableLines)

          this.doc.text(pageText, this.margin, currentY)

          if (remainingText.length > 0) {
            this.addNewPage()
            currentY = 70
          } else {
            currentY += pageText.length * 5 + 15
          }
        }
      } else {
    ...
    const generatedDate = new Date(narrative.generatedAt).toLocaleString()
    this.doc.setFontSize(8)
    this.doc.setTextColor(128, 128, 128)
    this.doc.text(
      `Report generated by AI on ${generatedDate}`,
