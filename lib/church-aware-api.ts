...
class ChurchAwareApiClient {
  ...
  private onAuthError?: (status: number, message: string) => void;
  ...
  setOnAuthError(callback: (status: number, message: string) => void) {
    this.onAuthError = callback;
  }

  private handleAuthError(status: number, message: string) {
    if (this.onAuthError) {
      this.onAuthError(status, message);
    } else {
      // Default behavior: redirect to login for 401, show alert for 403
  ...
  async request<T = any>(
    ...
    try {
      ...
      // Check for HTTP errors and handle auth errors
      if (!response.ok) {
      ...
      return {
        ...
        error: response.ok
          ? undefined
          : responseData?.error || responseData || "Request failed",
      };
    } catch (error) {
      // Check for auth errors (even though fetch threw, we might have status info)
      if (
  ...
  async get<T = any>(
    url: string,
    options?: Omit<ChurchAwareApiOptions, "method">,
  ): Promise<ApiResponse<T>> {
  ...
  async post<T = any>(
    url: string,
    data?: any,
    options?: Omit<ChurchAwareApiOptions, "method" | "body">,
  ): Promise<ApiResponse<T>> {
