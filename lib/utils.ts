 ...
 */
export function formatCurrency(amount: number | string | readonly (string | number)[] | undefined | null): string {
  let numericValue: number

  if (Array.isArray(amount)) {
    // Take the first element if it's an array
    const firstVal = amount[0]
    numericValue = typeof firstVal === 'number' ? firstVal : parseFloat(String(firstVal || 0))
  } else if (typeof amount === 'number') {
    numericValue = amount
  } else {
    numericValue = parseFloat(String(amount || 0))
  }
