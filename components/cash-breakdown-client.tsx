...
import type { CashBreakdownData } from '@/lib/server-data'
...
export default function CashBreakdownClient({ initialData }: CashBreakdownClientProps) {
  ...
  useEffect(() => {
    const initialFundData: FundData = {}
    FUND_TYPES.forEach(fund => {
      initialFundData[fund] = {}
      DENOMINATIONS.forEach(denom => {
        initialFundData[fund][denom] = 0
      })
    })

    // Populate with initial data
    initialData.forEach((item: CashBreakdownData) => {
      if (initialFundData[item.fund_type]) {
        initialFundData[item.fund_type][item.denomination] = item.count
      }
    })

    setFundData(initialFundData)
  }, [initialData])
      ...
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between animate-fade-in animate-slide-in-from-top-4 animate-duration-700">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={saveData} disabled={saving} className="transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {FUND_TYPES.map((fundType, index) => (
            <Card key={fundType} className={`hover:bg-accent/50 transition-all duration-300 hover:scale-105 animate-fade-in animate-slide-in-from-bottom-4 animate-duration-700`} style={{ animationDelay: `${800 + index * 100}ms` }}>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground">{fundType}</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card className={`bg-income/10 border-income/30 hover:bg-income/15 transition-all duration-300 hover:scale-105 animate-fade-in animate-slide-in-from-bottom-4 animate-duration-700`} style={{ animationDelay: `${800 + FUND_TYPES.length * 100}ms` }}>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-income">Total Cash</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <div className="space-y-6">
          {FUND_TYPES.map((fundType, index) => (
            <Card key={fundType} className={`transition-all duration-300 animate-fade-in animate-slide-in-from-bottom-4 animate-duration-700`} style={{ animationDelay: `${1200 + index * 200}ms` }}>
              <CardHeader>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border bg-muted/50">
                        <th className="text-left p-3 text-muted-foreground font-medium">Denomination</th>
                        <th className="text-left p-3 text-muted-foreground font-medium">Count</th>
                        <th className="text-right p-3 text-muted-foreground font-medium">Total Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      {DENOMINATIONS.map(denomination => {
                        ...
                        return (
                          <tr key={denomination} className="border-b border-border hover:bg-accent/50 transition-colors duration-200">
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-border font-bold bg-muted/50">
                        <td className="p-3 text-foreground">Subtotal</td>
                        <td className="p-3"></td>
        ...
        {/* Grand Total */}
        <Card className={`bg-income/10 border-income/30 hover:bg-income/15 transition-all duration-500 hover:scale-[1.02] animate-fade-in animate-slide-in-from-bottom-4 animate-duration-700`} style={{ animationDelay: `${1200 + FUND_TYPES.length * 200 + 200}ms` }}>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-bold text-income flex items-center gap-3">
                <Calculator className="h-7 w-7 text-income" />
                Grand Total
              </h3>
              <div className="text-4xl font-bold text-income">
