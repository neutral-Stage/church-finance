    ...
    try {
      ...
      if (response.ok) {
        ...
        if (response.status === 401) {
          console.log(
            "ChurchSelector: User not authenticated, setting empty churches list",
          );
        }
  ...
  return (
    <>
      ...
      <div aria-live="polite" aria-atomic="true" className="sr-only">
        {currentChurch
          ? `Selected church: ${currentChurch.name}`
          : "No church selected"}
      </div>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            className="justify-between bg-slate-800 border-slate-700 hover:bg-slate-700 text-white w-full max-w-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"
            aria-label={
              currentChurch
                ? `Selected church: ${currentChurch.name}. Click to change`
                : "Select a church"
            }
        ...
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-2xl">
          ...
          <div className="grid gap-3 max-h-96 overflow-y-auto">
            {churches.map((church) => (
              <Card
                key={church.id}
                className={`cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${
                ...
                role="button"
                tabIndex={0}
                aria-selected={currentChurch?.id === church.id}
              >
                <CardContent className="p-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center space-x-2 mt-1">
                          ...
                          <Badge
                            className={getRoleColor(church.role?.name || "")}
                          >
                            {church.role?.display_name || "No Role"}
                          </Badge>
                        </div>
                  ...
                  <div className="flex items-center space-x-4 mt-3 text-xs text-gray-400">
                    <div className="flex items-center space-x-1">
                      ...
                      <span>
                        Access granted{" "}
                        {church.user_church_role?.granted_at
                          ? new Date(
                              church.user_church_role.granted_at,
                            ).toLocaleDateString()
                          : "Unknown"}
