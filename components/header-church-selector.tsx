...
export function HeaderChurchSelector({ className }: HeaderChurchSelectorProps) {
  ...
  if (isLoading) {
    return (
      <div
        className={`flex items-center space-x-2 px-3 py-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg ${className}`}
      >
  ...
  if (error) {
    return (
      <div
        className={`flex items-center space-x-2 px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg ${className}`}
      >
    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            className={`justify-between bg-white/5 border-white/10 hover:bg-white/10 text-white backdrop-blur-sm transition-colors h-auto py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 ${className}`}
            aria-label={
              selectedChurch
                ? `Selected church: ${selectedChurch.name}. Click to change`
                : "Select a church"
            }
        ...
        <DialogContent className="bg-black/90 backdrop-blur-xl border-white/10 text-white max-w-2xl">
          ...
          <div className="grid gap-3 max-h-96 overflow-y-auto">
            {availableChurches.map((church) => (
              <Card
                key={church.id}
                className={`cursor-pointer transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${
                ...
                role="button"
                tabIndex={0}
                aria-selected={selectedChurch?.id === church.id}
              >
