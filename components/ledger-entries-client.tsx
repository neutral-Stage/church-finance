...
const ComprehensiveLedgerDialog = dynamic(
  () =>
    import("@/components/comprehensive-ledger-dialog").then(
      (mod) => mod.ComprehensiveLedgerDialog,
    ),
  {
    ssr: false,
    loading: () => (
...
export function LedgerEntriesClient({
  initialData,
  permissions,
}: LedgerEntriesClientProps): JSX.Element {
  const [ledgerEntries, setLedgerEntries] =
    useState<LedgerEntryWithRelations[]>(initialData);
  const [isComprehensiveDialogOpen, setIsComprehensiveDialogOpen] =
    useState(false);
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await fetch(
        "/api/ledger-entries?include_subgroups=true&include_bills=true",
        {
          method: "GET",
          credentials: "include",
          headers: {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to load ledger entries data";
    ...
    const setupRealtimeSubscription = async () => {
      try {
        ...
        subscription = supabase
          ...
          .on(
            "postgres_changes",
            { event: "*", schema: "public", table: "ledger_subgroups" },
            () => {
              fetchData();
            },
          )
        ...
        console.warn(
          "Failed to setup realtime subscription for ledger entries:",
          error,
        );
  ...
  const deleteEntry = async (entryId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this ledger entry? This will also delete all associated subgroups and bills.",
      )
    )
      return;
  ...
  const deleteSubgroup = async (subgroupId: string) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this subgroup? This will also delete all associated bills.",
      )
    )
      return;
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to delete subgroup";
      toast.error(errorMessage);
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge
        className={`${config.color} bg-white/10 backdrop-blur-xl border flex items-center gap-1`}
      >
  ...
  const getPriorityBadge = (priority: string) => {
    ...
    return (
      <Badge
        className={`${priorityConfig[priority as keyof typeof priorityConfig] || priorityConfig.medium} bg-white/10 backdrop-blur-xl border`}
      >
        {priority}
      </Badge>
    );
  ...
  const totalSubgroups = ledgerEntries.reduce(
    (sum, entry) => sum + (entry.ledger_subgroups?.length || 0),
    0,
  );
  const totalBills = ledgerEntries.reduce((sum, entry) => {
    const entryBills = entry.bills?.length || 0;
    const subgroupBills =
      entry.ledger_subgroups?.reduce(
        (subSum, subgroup) => subSum + (subgroup.bills?.length || 0),
        0,
      ) || 0;
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-4xl font-bold text-white">Ledger Entries</h1>
            <p className="text-white/70 mt-2">
              Manage hierarchical bill grouping and organization
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <Card
            className="bg-white/10 backdrop-blur-xl border border-white/20 hover:bg-white/15 hover:scale-105 transition-all duration-300 animate-fade-in animate-slide-in-from-bottom-4"
          ...
          {ledgerEntries.length === 0 ? (
            <Card className="bg-white/10 backdrop-blur-xl border border-white/20 hover:bg-white/15 transition-all duration-300">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-white/70 mb-6">
                  Create your first ledger entry to start organizing bills
                </p>
                {permissions.canEdit && (
            ...
            ledgerEntries.map((entry, index) => {
              ...
              return (
                <Card
                  key={entry.id}
                  className="bg-white/10 backdrop-blur-xl border border-white/20 hover:bg-white/15 transition-all duration-300 animate-fade-in animate-slide-in-from-bottom-4"
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-sm text-white/80">
                      ...
                      {entry.total_amount && (
                        <div className="flex items-center gap-2">
                          <span>
                            Estimated: {formatCurrency(entry.total_amount)}
                          </span>
                        </div>
                      )}
                      {entry.responsible_parties &&
                        entry.responsible_parties.length > 0 && (
                          <div className="flex items-center gap-2">
                        ...
                        {directBills.length > 0 && (
                          <div>
                            <h4 className="font-medium text-white mb-2">
                              Direct Bills ({directBills.length})
                            </h4>
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center justify-between mb-2">
                                      ...
                                      <div className="flex items-center gap-2">
                                        {getStatusBadge(
                                          subgroup.status || "draft",
                                        )}
                                        {getPriorityBadge(
                                          subgroup.priority || "medium",
                                        )}
                                        {permissions.canEdit && (
                                          <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                    ...
                                    {subgroupBills.length > 0 && (
                                      <div className="mt-3">
                                        <p className="text-sm font-medium text-white/80 mb-2">
                                          Bills ({subgroupBills.length})
                                        </p>
                                        <div className="space-y-2">
                                          {subgroupBills.map((bill) => (
                                            <div
                                              key={bill.id}
                                              className="flex items-center justify-between p-2 bg-white/5 backdrop-blur-sm border border-white/10 rounded"
                                            >
                                              <div>
                                                <p className="text-sm font-medium text-white">
                                                  {bill.vendor_name}
