...
export default function NotificationsDropdown({
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      const fetchedNotifications =
        await ClientNotificationService.getUserNotifications({ limit: 20 });
      setNotifications(fetchedNotifications);
      setUnreadCount(
        ClientNotificationService.getUnreadCount(fetchedNotifications),
      );
    } catch (err) {
        ...
        try {
          unsubscribe = ClientNotificationService.subscribeToNotifications(
            ...
            (updatedNotifications) => {
              ...
              setUnreadCount(
                ClientNotificationService.getUnreadCount(updatedNotifications),
              );
            },
          );
        } catch (err) {
  ...
  return (
    <>
      {/* Live region for screen readers - announces notification changes */}
      <div
        aria-live="polite"
        aria-atomic="true"
        className="sr-only"
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className={`relative text-white/70 hover:text-white hover:bg-white/10 hover:scale-105 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 ${className}`}
            aria-label={`Notifications${unreadCount > 0 ? `, ${unreadCount} unread` : ""}`}
            aria-describedby="notifications-live-region"
          >
          ...
          <div className="p-4 border-b border-white/10 bg-gradient-to-r from-white/5 to-white/10">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Badge
                  variant="secondary"
                  className="bg-transparent border border-white/20 text-white/80 backdrop-blur-sm"
                >
                  {unreadCount} new
                </Badge>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-start space-x-3">
                        <div
                          className={`relative p-2 rounded-full ${categoryColor} flex-shrink-0 backdrop-blur-sm border border-white/20 transition-all duration-300`}
                        >
                          <div
                            className={`absolute inset-0 rounded-full blur-md ${categoryColor?.replace?.("bg-", "bg-")?.replace?.("-500", "-400/20") || "bg-gray-400/20"}`}
                          />
                          <div className="relative">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h4
                                className={`text-sm font-semibold transition-colors ${!notification.read ? "text-white group-hover:text-white" : "text-white/80 group-hover:text-white/90"}`}
                              >
                                {notification.title}
                              </h4>
                              <p
                                className={`text-sm mt-1 transition-colors ${!notification.read ? "text-white/80 group-hover:text-white/90" : "text-white/60 group-hover:text-white/70"}`}
                              >
                              ...
                              {!notification.read && (
                                <div className="flex items-center mt-2">
                                  ...
                                  <span className="ml-2 text-blue-400 text-xs font-medium">
                                    New
                                  </span>
                                </div>
                              )}
